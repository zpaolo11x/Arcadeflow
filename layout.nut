...
function z_list_updategamedata(index){
	...
	dat.but_array[dat.stacksize - 1].file_name = (AF.folder+"button_images/" + z_list.boot[index].z_buttons+"button.png")
	dat.ply_array[dat.stacksize - 1].file_name = (AF.folder+"players_images/players_" + z_list.boot[index].z_players+".png")
	dat.ctl_array[dat.stacksize - 1].file_name = (AF.folder+controller_pic (z_list.boot[index].z_control))
...
function update_snapcrop (i,var,indexoffsetvar,indexvar,aspect,cropaspect){
	...
	if (prf.BOXARTMODE && (tilez[i].gr_snapz.texture_width == 0 )) {
		...
		if (tilez[i].gr_snapz.texture_width == 0 ) tilez[i].gr_snapz.file_name = "category_images_10/grey.png"
