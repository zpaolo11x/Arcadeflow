...
function cleanWHDL(inputname){
   ...
   local prestring = inputname.slice(0,vstart)
   local poststring = inputname.slice(vstart+1)
   local vstop = poststring.find("_")
   if (vstop != null) {
      poststring = poststring.slice(vstop+1)
      outputname = prestring+"_"+poststring
   }
   else outputname = prestring
   return outputname
...
function collapsetable(tablein,tablecriteria){
   local out = {}
   foreach (i, item in tablein){
      out[item[tablecriteria]] <- item.text
   }
   return (out)
}
...
function parsejson(scrapeid, gamedata){
   ...
   local regionprefs = gamedata.regionprefs

   // ** regionprefs is the region preference list as set in Arcadeflow

   local romregionsarray = [""]
   local romregions = ""
   try {romregions = jstab.response.jeu.rom.romregions} catch (err){}
   try {romregionsarray = split(romregions,", ")} catch (err){}

   //** romregions are the roms data from the jstab.response.jeu.rom.romregions in the json
   ...
   // Assign romregions and romregionsarray to the proper source (filename, if present, rules over metadata)
   if (filenameregions != "") {
      romregions = filenameregions
      romregionsarray = filenameregionsarray
   }

   // Now sort romregions using regionprefs array as reference.
   local tempromregions = []
   local tempromregions2 = []
   foreach (i, item in regionprefs){
      if (romregions.find(item) != null) tempromregions.push(item)
      else tempromregions2.push(item)
   }
   foreach (i, item in tempromregions2){
      tempromregions.push (item)
   }
   romregionsarray = tempromregions

   local langtouse = "en"

   // Gets the regions for the current rom
   gamedata.regions = romregions
   // Collapses the name by region table, scans the rom regions to find the proper name
   // For multi-region roms, it gets the first by default (so eu,jp will return eu if present)
   // This can be changed in the future to allow region preferences for multiple rom regions
   gamedata.title = filtertable(collapsetable(jstab.response.jeu.noms,"region"),romregionsarray,false)
   // If no proper name is found with the correct rom region, search for "ss" game name
   if (gamedata.title == "") {
   ...
   try {
      foreach (i, item in jstab.response.jeu.familles){
         foreach (i2, item2 in item.noms){
            ...
            if (i2 == langtouse) break
         }
      }
   } catch (err){}
   // Scans all the genres in the selected language, by preference it selects the first genre with "/" structure
   ...
   if (gamedata.isarcade){
      ...
      if (a_controls_array.len()<2) a_controls_array = split (a_controls,"|")
   ...
   foreach (id,item in jstab.response.jeu.medias){
      try {
         ...
         if (catpriority == 0) gamedata.media[catname].push(tempdata)
         else gamedata.media[catname].insert(0,tempdata)
      }
      catch(err){}
   }
   ...
   foreach (id, item in gamedata.media){
      if (item.len() > 1) { // There is more than one media in this category
         ...
         gamedata.media[id] = filteredarray
      }
   }
...
function getromcrc_halfbyte(filepath){

   local Crc32Lookup16 = [0x00000000,0x1DB71064,0x3B6E20C8,0x26D930AC,0x76DC4190,0x6B6B51F4,0x4DB26158,0x5005713C,0xEDB88320,0xF00F9344,0xD6D6A3E8,0xCB61B38C,0x9B64C2B0,0x86D3D2D4,0xA00AE278,0xBDBDF21C]
...
local Crc32Lookup4 = [[0x00000000,0x77073096,0xEE0E612C,0x990951BA,0x076DC419,0x706AF48F,0xE963A535,0x9E6495A3,
...
function getromcrc_lookup(filepath){
   ...
   local crc = 0xFFFFFFFF
   while (i<blb.len()) {
      crc = (crc >> 8) ^ Crc32Lookup[(crc & 0xFF) ^ blb[i++]]
   }
   crc = ~crc
...
function getromcrc_lookup4(filepath){
   ...
   while (i<blb.len()) {
      byte = blb.readn('b')
      crc = (crc >> 8) ^ Crc32Lookup4[0][(crc & 0xFF) ^ byte]
      i = i + 1
   }
   crc = ~crc
...
function getromcrc_old(filepath){
   ...
   local output = [format("%X",crc).slice(-8),format("%x",crc).slice(-8)]
   return output
}