...
{   
   ...
   //vec3 hsl1 = vec3 (103.0/360.0, 0.97, 0.29);
   //vec3 hsl2 = vec3 (67.0/360.0, 0.99, 0.58);

    vec3 hsl1 = rgb2hsv(color1);
   vec3 hsl2 = rgb2hsv (color2);


   vec2 uv = gl_TexCoord[0].xy;
   vec2 uvpix = (mod(pixelsize * uv , 1.0));
   
   uv = (floor (pixelsize * uv) + 0.5) / pixelsize;

   vec4 col = texture2D(texture, uv.xy);

   //TEST97 if (lcdcolor) col = gbacolor(col);
   col = mix ( col, gbacolor(col),lcdcolor);
   vec4 colpix = texture2D(pixel, uvpix.xy);

   col.rgb = col.rgb + plusminus * matrix * 0.25*(1.0-colpix.rgb);

   //if (remap) col.rgb = vec3(mix(color1.rgb,color2.rgb,col.x));
   //TEST97 if (remap) col.rgb = hsv2rgb(vec3(mix(hsl1.rgb,hsl2.rgb,col.x)));
    col.rgb = mix (col.rgb, hsv2rgb(vec3(mix(hsl1.rgb,hsl2.rgb,col.x))), remap);
