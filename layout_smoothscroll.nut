...
local textbox = fe.add_text("test", 0, 0, flw * 0.5, flh *0.5)
textbox.charsize = textbox.height * 0.711 / 10
...
function on_signal(sig) {
	if (sig == "up") {
		moveshift = moveshift - textbox.charsize
		return true
	}
	if (sig == "down") {
		moveshift = moveshift + textbox.charsize
...
function tick(tick_time) {
	if (moveshift != 0) {
		if (moveshift > 0) {
			textbox.y ++
			moveshift --
			if (moveshift % textbox.charsize == 0) {
